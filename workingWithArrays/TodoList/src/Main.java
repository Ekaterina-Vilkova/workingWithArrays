...
//        String DELETE; // удаляет; если указан несуществующий индекс - ничего не делать.
        ...
        while (true) {
            ...
            if (word[0].equals("ADD")) {
                ...
                if (withoutFirstWord.length > 0 && withoutFirstWord[0].matches("[0-9]")) {
            ...
            if (word[0].equals("EDIT") && word[1].matches("[0-9]")) {
                ...
                result = "Дело" + " \"" + previousDeal + "\" " + "заменено на " + "\"" + textWithoutFirstWord.trim() + "\" ";
                System.out.println(result);
            }
            if (word.length == 2 && word[0].equals("DELETE") && word[1].matches("[1-9][0-9]{2}")) {
